...
Derived from `AI_Relationship_Intelligence_Analysis.md`. Schema already applied
(`schema_relationship_intelligence.sql`, verified live in the DB). This file
breaks that vision into ordered, independently-shippable phases — pick one,
implement it, verify it, move to the next. Nothing here is implemented yet.
